 ...
 * `select key from email_templates` and re-verified 2026-08-10 after the support
 * migration was applied. Checked in so the "no dead rows" claim is a test rather
 * than a sentence in a ledger.
 *
 * A snapshot, deliberately: the alternative is a test that queries production,
 * which would pass on a laptop with a working key and fail in CI, and would stop
...
const PRODUCTION_TEMPLATE_KEYS = [
  ...
  "customer_message", "fulfillment_processing", "guest_order_access", "low_stock_alert", "needs_information",
...
test("the variable list is derived, and covers every usable variable", () => {
  ...
  // 14 → 16 in the support pass: `support_reference` (generated by a sequence)
  // and `support_subject` (the customer's own words, returned to the customer
  // who typed them). The count is asserted so a variable cannot be added without
  // somebody deciding it is safe to interpolate into a customer email.
  assert.equal(CUSTOMER_SAFE_VARIABLES.length, 16);
  assert.ok((CUSTOMER_SAFE_VARIABLES as readonly string[]).includes("support_reference"));
  assert.ok((CUSTOMER_SAFE_VARIABLES as readonly string[]).includes("support_subject"));
});
