...
test("customer navigation exposes projects and community without workshop", () => { const header = read("src/components/SiteHeader.tsx"); for (const route of ["/about", "/capabilities", "/projects", "/catalog", "/contact", "/community"]) assert.match(header, new RegExp(route)); assert.match(header, /href: "\/projects", label: "Projects"/); assert.doesNotMatch(header, /Community is coming soon|aria-disabled="true"/); assert.doesNotMatch(header.match(/const leftLinks[\s\S]*?const desktopPillBase/)?.[0] || "", /Workshop/); });
