...
test("customer navigation prioritizes the storefront journey", () => { const header = read("src/components/SiteHeader.tsx"); for (const route of ["/about", "/capabilities", "/catalog", "/contact"]) assert.match(header, new RegExp(route)); assert.doesNotMatch(header.match(/const leftLinks[\s\S]*?const desktopPillBase/)?.[0] || "", /Community|Workshop|Info/); });
