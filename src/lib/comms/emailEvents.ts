...
export const CUSTOMER_SAFE_VARIABLES = [
  ...
  "date",
  /*
   * Support. Both are safe by construction rather than by review:
   *
   * - `support_reference` is generated by a Postgres sequence — `SUP-0007` — and
   *   contains nothing a person wrote.
   * - `support_subject` is the customer's **own** subject line, returned to the
   *   customer who typed it. It reaches the subject header, where
   *   `interpolateHeader` strips line breaks, and the body, where `escapeHtml`
   *   runs. It is never shown to a different customer.
   */
  "support_reference",
  "support_subject",
] as const;
...
export const EMAIL_EVENTS: readonly EmailEvent[] = [
  ...
  // ------------------------------------------------------------------ support
  //
  // Four events, and the thing to notice is what is **absent**: there is no
  // event for an internal note. That is not an omission — the internal-note
  // route makes no call to `sendCommerceEmail` at all, so there is no send to
  // suppress and no flag that could be set wrongly. A note reaches a customer
  // only if somebody writes a new code path, and the matrix test would fail on
  // an unlisted send.
  {
    id: "support_received",
    ...
    audience: "customer",
    related: "support",
    trigger: "A support conversation is opened at POST /api/support, by an account holder or a guest.",
    eventKeyShape: "support-received-{conversationId}",
    idempotency: "suppressed",
    activity: "support_conversations + support.created",
    resendable: true,
    wired: true,
  },
  {
    ...
    related: "support",
    trigger: "The same submission, to the configured staff alert address.",
    eventKeyShape: "support-received-staff-{conversationId}",
    idempotency: "suppressed",
    activity: "support_conversations + support.created",
    resendable: false,
    wired: true,
  },
  {
    ...
    audience: "customer",
    related: "support",
    trigger: "Staff post a customer-visible reply on a support conversation.",
    // Keyed on the message, because a second reply is a genuinely new event and
    // not a duplicate of the first. The message id is minted once per row, and
    // the client token collapses a double-submitted reply into one row — so the
    // dedup happens at the row, and the email inherits it.
    eventKeyShape: "support-reply-{messageId}",
    idempotency: "per_message",
    activity: "support_messages + support.staff_replied",
    resendable: true,
    wired: true,
  },
  {
    ...
    audience: "customer",
    related: "support",
    trigger: "Staff move a support conversation to resolved.",
    // Keyed on the transition, not the conversation: a conversation that is
    // resolved, reopened and resolved again has genuinely been resolved twice,
    // and the customer should be told each time. The audit event id is what
    // makes the second one a new key.
    eventKeyShape: "support-resolved-{conversationId}-{resolvedAt}",
    idempotency: "suppressed",
    activity: "support.resolved",
    resendable: true,
    wired: true,
  },
