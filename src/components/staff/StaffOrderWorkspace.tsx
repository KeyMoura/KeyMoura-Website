...
export function StaffOrderWorkspace({ orderId, canManage }: { orderId:string; canManage:boolean }) {
  ...
  return <section className="ui-card print:border-0 print:bg-white print:p-0 print:text-black">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="ui-eyebrow">Workshop</p><h2 className="mt-1 text-xl font-semibold">Production workspace</h2><p className="mt-1 text-xs text-brand-textMuted print:text-zinc-600">Private job planning, tasks, materials, and costs.</p></div><button type="button" onClick={()=>window.print()} className="ui-btn ui-btn-secondary text-xs print:hidden">Print job sheet</button></div>
    ...
    {canManage?<button disabled={saving} onClick={()=>void action({ action:"save_workspace", priority:workspace.priority, assigned_to:workspace.assigned_to, started:Boolean(workspace.started_at) })} className="ui-btn ui-btn-primary mt-3 text-sm disabled:opacity-50 print:hidden">Save planning</button>:null}

    <div className="mt-6 grid gap-6 xl:grid-cols-2 print:grid-cols-2">
      <div><div className="flex items-center justify-between"><h3 className="font-semibold">Production checklist</h3><span className="text-xs text-brand-textMuted">{complete}/{checklist.length} done</span></div>{checklist.length?<div className="mt-3 space-y-2">{checklist.map(item=><div key={item.id} className="ui-card flex items-center gap-3 !p-3"><input disabled={!canManage} type="checkbox" checked={item.is_complete} onChange={event=>void action({action:"toggle_checklist",item_id:item.id,is_complete:event.target.checked})}/><span className={`min-w-0 flex-1 text-sm ${item.is_complete?"text-brand-textMuted line-through":""}`}>{item.title}</span>{canManage?<button onClick={()=>void action({action:"delete_checklist",item_id:item.id})} className="text-xs text-rose-300 print:hidden" aria-label={`Remove ${item.title}`}>Remove</button>:null}</div>)}</div>:<EmptyState className="mt-3">No production steps yet.</EmptyState>}{canManage?<form onSubmit={addTask} className="mt-3 flex gap-2 print:hidden"><input required maxLength={240} value={task} onChange={event=>setTask(event.target.value)} className={`${input} min-w-0 flex-1`} placeholder="Add a production step…"/><button disabled={saving} className="ui-btn ui-btn-secondary text-sm">Add</button></form>:null}</div>
