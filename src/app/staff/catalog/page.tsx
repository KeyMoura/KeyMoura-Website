            ...
            <div className="mt-4 space-y-4">
              {groups.map((group, groupIndex) => <div key={group.id} className="rounded-xl border border-zinc-800 p-4">
                <div className="grid gap-3 md:grid-cols-4">
                  ...
                  <select className={input} value={group.input_type} onChange={e => setGroups(current => current.map(item => item.id === group.id ? { ...item, input_type: e.target.value as ProductOptionGroup["input_type"] } : item))}>
                    <option value="select">Dropdown</option><option value="radio">Choice cards</option><option value="text">Short text</option><option value="textarea">Long text</option><option value="number">Number</option><option value="checkbox">Checkbox</option><option value="file">Reference file</option>
                  </select>
