...
export default function StaffCatalogPage(){
 ...
 {canManage?<form onSubmit={create} className="mt-6 grid gap-3 rounded-2xl border border-zinc-800 bg-black/30 p-5 sm:grid-cols-2"><input required className={input} value={name} onChange={e=>setName(e.target.value)} placeholder="Product name"/><input className={input} value={category} onChange={e=>setCategory(e.target.value)} placeholder="Category"/><input className={input} type="number" min="0" step=".01" value={price} onChange={e=>setPrice(e.target.value)} placeholder="Starting price (optional)"/><input className={input} value={description} onChange={e=>setDescription(e.target.value)} placeholder="Short description"/><button className="rounded-xl border border-brand-primary/80 bg-brand-primary/20 px-4 py-2 font-semibold text-brand-primary transition hover:bg-brand-primary/30 sm:col-span-2">Create draft product</button></form>:null}
