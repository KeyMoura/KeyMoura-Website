...
export default function AnalyticsPage() {
  ...
  return <main className="page-stack mx-auto w-full max-w-7xl">
    <div className="flex flex-wrap items-end justify-between gap-4">
      ...
      <SegmentedControl value={range} onChange={setRange} ariaLabel="Analytics date range" options={[{ value: "30d", label: "30 days" }, { value: "90d", label: "90 days" }, { value: "all", label: "All time" }]} />
    </div>

    {error ? <Notice tone="danger" role="alert">{error}</Notice> : null}
    {loading && !summary ? <EmptyState>Loading business analytics…</EmptyState> : null}

    {summary ? <>
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Financial summary">
        <MetricCard label="Net collected" value={money(summary.netCollectedCents)} detail={`${money(summary.grossCollectedCents)} gross · ${money(summary.refundedCents)} refunded`} />
        <MetricCard label="Outstanding" value={money(summary.outstandingCents)} detail="Quoted balance remaining on active orders" tone={summary.outstandingCents > 0 ? "warning" : "default"} />
        <MetricCard label="Average paid order" value={money(summary.averageOrderCents)} detail="Net collected per order with payment" />
        <MetricCard label="Active workload" value={String(summary.activeOrderCount)} detail={`${summary.overdueCount} overdue · ${summary.agingCount} untouched for 14+ days`} tone={summary.overdueCount > 0 || summary.agingCount > 0 ? "warning" : "default"} />
      </section>
      ...
      <section className="grid gap-5 xl:grid-cols-2">
        <Panel><h2 className="text-lg font-semibold">Order pipeline</h2><p className="mt-1 text-xs text-brand-textMuted">Requests created in this period by current status</p><div className="mt-5 space-y-3">{summary.pipeline.map((stage) => <div key={stage.status}><div className="flex justify-between gap-3 text-sm"><span>{pretty(stage.status)}</span><span className="font-semibold">{stage.count}</span></div><div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-brand-accent" style={{ width: `${Math.max(8, stage.count / maxPipeline * 100)}%` }} /></div></div>)}</div>{!summary.pipeline.length ? <EmptyState className="mt-5">No orders in this period.</EmptyState> : null}</Panel>
        <Panel><h2 className="text-lg font-semibold">Customer health</h2><p className="mt-1 text-xs text-brand-textMuted">How broad and repeatable demand is becoming</p><dl className="mt-5 grid grid-cols-2 gap-3"><div className="ui-card"><dt className="text-xs text-brand-textMuted">Unique customers</dt><dd className="mt-1 text-2xl font-semibold">{summary.uniqueCustomers}</dd></div><div className="ui-card"><dt className="text-xs text-brand-textMuted">Repeat customers</dt><dd className="mt-1 text-2xl font-semibold">{summary.repeatCustomers}</dd></div></dl><p className="mt-4 text-xs text-brand-textMuted">A repeat customer has submitted more than one order during the selected period.</p></Panel>
      </section>

      <section className="grid gap-5 xl:grid-cols-2">
        <Panel><div className="flex justify-between gap-3"><div><h2 className="text-lg font-semibold">Most requested products</h2><p className="mt-1 text-xs text-brand-textMuted">Ranked by order count, including custom requests</p></div><Link href="/staff/catalog" className="text-xs font-medium text-brand-accent hover:underline">Catalog</Link></div><div className="mt-4 divide-y divide-white/10">{summary.topProducts.map((product) => <div key={product.name} className="grid grid-cols-[1fr_auto] gap-4 py-3"><div><p className="text-sm font-medium">{product.name}</p><p className="mt-1 text-xs text-brand-textMuted">{product.units} unit{product.units === 1 ? "" : "s"} · {money(product.revenueCents)} net</p></div><Badge>{product.orders}</Badge></div>)}</div>{!summary.topProducts.length ? <EmptyState className="mt-5">No product demand yet.</EmptyState> : null}</Panel>
        <Panel><div className="flex justify-between gap-3"><div><h2 className="text-lg font-semibold">Inventory risk</h2><p className="mt-1 text-xs text-brand-textMuted">Tracked items at or below their warning threshold</p></div><Link href="/staff/catalog" className="text-xs font-medium text-brand-accent hover:underline">Restock</Link></div><div className="mt-4 divide-y divide-white/10">{summary.inventoryAlerts.slice(0, 8).map((product) => <div key={product.id} className="flex items-center justify-between gap-4 py-3"><div><p className="text-sm font-medium">{product.name}</p><p className="mt-1 text-xs text-brand-textMuted">Alert at {product.low_stock_threshold} · {product.is_published ? "Published" : "Draft"}</p></div><Badge tone={product.inventory_quantity === 0 ? "danger" : "warning"}>{product.inventory_quantity === 0 ? "Out" : `${product.inventory_quantity} left`}</Badge></div>)}</div>{!summary.inventoryAlerts.length ? <EmptyState className="mt-5">No low-stock products.</EmptyState> : null}</Panel>
      </section>

      <Panel><h2 className="text-lg font-semibold">Customer search gaps</h2><p className="mt-1 text-xs text-brand-textMuted">A compact replacement for the old search-only analytics page: terms customers searched that returned no Info results.</p><div className="mt-4 flex flex-wrap gap-2">{data?.searchInsights.noResultTerms.map((term) => <Badge key={term.query}>{term.query} <span className="text-brand-textMuted">×{term.count}</span></Badge>)}</div>{!data?.searchInsights.noResultTerms.length ? <EmptyState className="mt-4">No repeated content gaps in the latest {data?.searchInsights.searchesRecorded ?? 0} recorded searches.</EmptyState> : null}</Panel>
