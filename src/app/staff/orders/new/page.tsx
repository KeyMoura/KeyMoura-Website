...
export default function NewStaffOrderProposalPage() {
  ...
  const itemComplete = Boolean(customerId && productName.trim());
  const termsComplete = Boolean(Number(price) >= 0.5 && Number(quantity) > 0);
  return <main className="page-stack mx-auto max-w-4xl">
    ...
    <div className="ui-stepper" aria-label="Proposal progress"><div data-step="1" className={cx("ui-step", itemComplete ? "is-complete" : "is-current")}>Customer & item</div><div data-step="2" className={cx("ui-step", termsComplete ? "is-complete" : itemComplete ? "is-current" : "")}>Price & delivery</div><div data-step="3" className={cx("ui-step", itemComplete && termsComplete ? "is-current" : "")}>Review & send</div></div>
