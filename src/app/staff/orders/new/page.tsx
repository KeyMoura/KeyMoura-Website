...
type Product = { id:string; name:string; starting_price_cents:number|null; is_published:boolean };
...
export default function NewStaffOrderProposalPage() {
  ...
  function chooseProduct(id:string) {
    ...
    if (product) { setProductName(product.name); if (product.starting_price_cents != null) setPrice((product.starting_price_cents/100).toFixed(2)); }
  }
    ...
    <form onSubmit={submit} className="mt-6 grid gap-5 lg:grid-cols-[1fr_.8fr]">
      <div className="space-y-5">
        <section className="ui-card space-y-4"><div><p className="text-xs font-semibold uppercase tracking-[.15em] text-brand-accent">1 · Customer & item</p><h2 className="mt-1 text-xl font-semibold">What are you offering?</h2></div>
          ...
          <label className="block text-sm">Catalog item (optional)<select value={productId} onChange={e=>chooseProduct(e.target.value)} className="ui-input mt-2 w-full"><option value="">Custom line item</option>{products.map(item=><option key={item.id} value={item.id}>{item.name}{item.is_published ? "" : " (draft)"}</option>)}</select></label>
        ...
        <section className="ui-card space-y-4"><div><p className="text-xs font-semibold uppercase tracking-[.15em] text-brand-accent">2 · Price & delivery</p><h2 className="mt-1 text-xl font-semibold">Set the terms</h2></div>
          <div className="grid gap-4 sm:grid-cols-3"><label className="text-sm">Quantity<input required type="number" min="1" max="1000" value={quantity} onChange={e=>setQuantity(e.target.value)} className="ui-input mt-2 w-full" /></label><label className="text-sm">Total price<input required type="number" min="0.50" step="0.01" value={price} onChange={e=>setPrice(e.target.value)} className="ui-input mt-2 w-full" placeholder="125.00" /></label><label className="text-sm">Deposit (optional)<input type="number" min="0.50" step="0.01" value={deposit} onChange={e=>setDeposit(e.target.value)} className="ui-input mt-2 w-full" placeholder="50.00" /></label></div>
