...
function nextAction(order: Order) {
  ...
  if (order.status === "needs_information") return "Waiting for details";
...
export default function StaffOrdersPage() {
  ...
  const counts = useMemo(() => ({
    attention: orders.filter(order => attentionStatuses.has(order.status)).length,
  ...
  return <main>
    ...
    <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {(["attention", "active", "completed", "all"] as const).map(item => <button key={item} onClick={() => setView(item)} className={`rounded-2xl border p-4 text-left transition ${view === item ? "border-brand-accent bg-brand-accent/10" : "border-brand-border bg-black/20 hover:border-brand-accent/60"}`}><div className="text-2xl font-semibold">{counts[item]}</div><div className={`mt-1 text-sm ${view === item ? "text-brand-accent" : "text-brand-textMuted"}`}>{item === "attention" ? "Needs attention" : pretty(item)}</div></button>)}
    </div>
