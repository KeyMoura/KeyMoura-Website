...
export async function POST(req: NextRequest) {
  ...
  const actorRole = actor.role;
    ...
    // Permission model:
    // - If you have the apply permission for this restriction kind, you may apply it directly.
    // - Otherwise, if you have the request permission, you may submit it for approval.
    const permApply =
      kindRaw === "site"
        ? "moderation.restrict"
        : kindRaw === "community"
    ...
    const canApply = actor.permissions.has(permApply as any);
    const canRequest = canApply || actor.permissions.has(permRequest as any);
