...
export default function OrderDetailPage() {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:py-12">
      ...
      {order.status === "final_review" ? <section className="mt-4 rounded-2xl border border-brand-accent/50 bg-brand-accent/10 p-5"><p className="text-xs font-semibold uppercase tracking-[.16em] text-brand-accent">Finished-product review</p><h2 className="mt-2 text-xl font-semibold">Your order is ready for approval</h2><p className="mt-2 text-sm text-brand-textMuted">Review KeyMoura’s latest photos and updates in the order conversation. Approving confirms the finished work and sends it to fulfillment.</p><button type="button" disabled={busy} onClick={()=>void approveFinishedOrder()} className="mt-4 rounded-xl border border-brand-accent/70 bg-brand-accent/15 px-5 py-2.5 font-semibold text-brand-accent disabled:opacity-50">{busy?"Approving…":"Approve finished order"}</button></section> : null}
