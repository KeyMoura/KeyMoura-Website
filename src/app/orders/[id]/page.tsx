...
type Order = { id:string; order_number:string|null; product_name:string; status:string; quantity:number; specifications:Record<string,unknown>; customer_notes:string|null; agreed_price_cents:number|null; payment_status:string; amount_paid_cents:number; target_date:string|null; created_at:string; };
...
export default function OrderDetailPage() {
  ...
  return <main className="mx-auto max-w-4xl px-4 py-10">
    ...
    <section className="mt-6 rounded-2xl border border-zinc-800 bg-black/30 p-5"><h2 className="font-semibold">Request details</h2><dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-brand-textMuted">Quantity</dt><dd>{order.quantity}</dd></div><RequestSpecifications specifications={order.specifications||{}} /></dl>{order.customer_notes?<p className="mt-4 whitespace-pre-wrap border-t border-zinc-800 pt-4 text-sm">{order.customer_notes}</p>:null}</section>
