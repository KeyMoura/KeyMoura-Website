...
export default function PublicGaragePage() {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (carError) {
          ...
          setErrorMessage("Failed to load public garage.");
          setState("error");
          return;
        }
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-8 text-brand-text">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[12px] text-brand-textMuted sm:text-sm">
              Browse public builds from the community. Click a car to see full details.
            </p>
          </div>
      ...
      {state === "loading" && (
        <section>
          <p className="text-[12px] text-brand-textMuted">Loading public cars…</p>
        </section>
      )}
      {/* Empty */}
      {state === "loaded" && !hasCars && (
        <section className="rounded-xl border border-dashed border-zinc-800/80 bg-black/30 p-6 text-[13px] text-brand-textMuted">
          <p className="font-medium text-brand-text">No public cars have been added yet.</p>
          <p className="mt-1">
        ...
        <div className="space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-sm font-semibold text-brand-text">
              {highlightText(title || "Untitled car", highlightTokens)}
            </h3>
            {car.is_primary && (
