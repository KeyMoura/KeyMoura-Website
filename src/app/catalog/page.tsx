...
type Product = Pick<CatalogProduct, "id" | "name" | "slug" | "short_description" | "image_url" | "category" | "starting_price_cents" | "is_custom" | "availability_status" | "lead_time_text" | "inventory_policy" | "inventory_quantity" | "continue_selling_when_out_of_stock">;
...
export default function CatalogPage() {
  ...
  useEffect(() => { void supabase.from("products").select("id,name,slug,short_description,image_url,category,starting_price_cents,is_custom,availability_status,lead_time_text,inventory_policy,inventory_quantity,continue_selling_when_out_of_stock").eq("is_published", true).is("archived_at", null).order("sort_order").order("created_at", { ascending: false }).then(({ data, error: e }) => { setProducts((data ?? []) as Product[]); setError(e?.message ?? ""); setLoading(false); }); }, [supabase]);
  ...
  return <main className="mx-auto max-w-6xl px-4 py-10 md:py-14">
    ...
    <div className="mt-5 flex items-center justify-between text-sm text-brand-textMuted"><p>{loading ? "Loading catalog…" : `${visible.length} ${visible.length === 1 ? "product" : "products"}`}</p><Link href="/orders" className="text-brand-primary hover:underline">My requests & orders</Link></div>
    ...
    <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{visible.map(product => <article key={product.id} className="group overflow-hidden rounded-2xl border border-zinc-800 bg-black/30 transition hover:-translate-y-1 hover:border-brand-primary/40">
      <Link href={`/catalog/${product.slug}`} aria-label={`View ${product.name}`}>{product.image_url ? <>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src={product.image_url} alt={product.name} loading="lazy" className="aspect-[4/3] w-full object-cover transition duration-300 group-hover:scale-[1.02]" />
